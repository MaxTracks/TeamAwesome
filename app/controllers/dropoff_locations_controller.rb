class DropoffLocationsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @dropoff_location.update(dropoff_location_params)
        format.html { redirect_to @dropoff_location, notice: 'Dropoff location was successfully updated.' }
  ...
  private
    ...
    def dropoff_location_params
      params.require(:dropoff_location).permit(:location, :latitude, :longitude, :details, :items)
    end
end
